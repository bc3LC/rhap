...
create_panel <- function() {
  ...
  mort <- tibble::as_tibble(dplyr::bind_rows(
    read_ascii(file_path = file.path(datadir, "/mort/IHME-GBD_2019_DATA-15769af1-1.csv"), ascii_cols = "location_name"),
  ...
  gdp <- read_ascii(file_path = file.path(datadir, "/socioeconomic/gdp.csv"), ascii_cols = "country_name") %>%
    ...
    dplyr::mutate(iso = tolower(iso)) %>%
    dplyr::mutate(
      iso = dplyr::if_else(country_name == "Romania", "rom", iso),
  ...
  preprocess_clima <- function(file_path, value_name) {
    ...
    data_long <- data %>%
      ...
      dplyr::mutate(
        year = as.numeric(sub("X", "", sub("\\..*", "", year_month))),
        iso = tolower(code),
        !!value_name := value
      ) %>%
  ...
  data_fin_ret <- data_fin %>%
    ...
    dplyr::mutate(
      BC_per_100k = dplyr::if_else(BC_per_100k == 0, 1E-9, BC_per_100k),
    ...
    dplyr::mutate(
      log_BC_per_100k = log(BC_per_100k),
    ...
    dplyr::mutate(continent = countrycode::countrycode(
      sourcevar = iso,
      origin = "iso3c",
