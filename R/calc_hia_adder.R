...
calc_hia_adder <- function() {
  ...
  # ----------------------------------------------
  fixed_fin <- plm::plm(
    log_Deaths_per_100k ~ log_PrimPM25_per_100k +
      log_NOx_per_100k +
      log_VOC_per_100k +
  ...
  data_adder <- data %>%
    ...
    dplyr::mutate(
      Deaths_per_100k = exp(log_Deaths_per_100k),
  ...
  # 1- YLLs
  fixed_fin_yll <- plm::plm(
    log_YLL_per_100k ~ log_PrimPM25_per_100k +
      log_NOx_per_100k +
      log_VOC_per_100k +
      log_gdppc_ppp_dol2011 +
      log_flsp,
  ...
  data_adder_yll <- data %>%
    ...
    dplyr::filter(year == max(year)) %>%
    dplyr::mutate(
      YLL_per_100k = exp(log_YLL_per_100k),
  ...
  # 2- DALYs
  fixed_fin_daly <- plm::plm(
    log_DALY_per_100k ~ log_PrimPM25_per_100k +
      log_NOx_per_100k +
      log_VOC_per_100k +
      log_gdppc_ppp_dol2011 +
      log_flsp,
  ...
  data_adder_daly <- data %>%
    ...
    dplyr::filter(year == max(year)) %>%
    dplyr::mutate(
      DALY_per_100k = exp(log_DALY_per_100k),
